...
export class VehicleRepository {
  ...
  async getAvailableVehiclesInDateRange(
    ...
    const { data: bookedVehicles, error: bookingError } = await this.supabase
      .from('reservations')
      .select('vehicle_id')
   ...
   */
  async createVehicle(vehicleData: any): Promise<void> {
    const { error } = await this.supabase.from('vehicles').insert([vehicleData]);
  ...
  /**
   * Sube la imagen del vehículo al Storage y retorna la URL pública
   */
   ...
   */
  async updateVehicle(id: string, vehicleData: any): Promise<void> {
    const { error } = await this.supabase
      .from('vehicles')
      .update(vehicleData)
      .eq('id', id);
