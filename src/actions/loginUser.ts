...
export async function loginUser(input: LoginInput): Promise<never> {
  ...
  try {
    ...
    const { user, redirectUrl } = await authService.login(validatedInput.email, validatedInput.password);
    ...
    await CookieManager.setTokenPair(accessToken, refreshToken, user.id);
